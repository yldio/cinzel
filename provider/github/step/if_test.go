...
func TestStepIfSuccess(t *testing.T) {
	type Test struct {
		name   string
		have   []byte
		expect cty.Value
	...
	var tests = []Test{
		{
			"test 1",
			[]byte(`"$${{ ! startsWith(github.ref, 'refs/tags/') }}"`),
			cty.StringVal("${{ ! startsWith(github.ref, 'refs/tags/') }}"),
			"if: ${{ ! startsWith(github.ref, 'refs/tags/') }}\n",
		},
		{
			"test 2",
			[]byte(`"$${{ failure() }}"`),
			cty.StringVal("${{ failure() }}"),
			"if: ${{ failure() }}\n",
		},
		{
			"test 3",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hv.Add("test", cty.StringVal("${{ failure() }}"))
			...
			if err := stepParsed.parseIf(hp.Result()); err != nil {
				t.FailNow()
			}

			if stepParsed.If != tt.expect {
				t.FailNow()
			}
...
func TestStepIfFailure(t *testing.T) {
	type Test struct {
		name   string
		have   []byte
		expect cty.Value
