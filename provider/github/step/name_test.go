...
func TestStepNameSuccess(t *testing.T) {
	type Test struct {
		name   string
		have   []byte
		expect cty.Value
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hv.Add("test", cty.StringVal("my-name"))
			...
			if err := stepParsed.parseName(hp.Result()); err != nil {
				t.FailNow()
			}

			if stepParsed.Name != tt.expect {
				t.FailNow()
			}
...
func TestStepNameFailure(t *testing.T) {
	type Test struct {
		name   string
		have   []byte
		expect cty.Value
